...
def naiveSoftmaxLossAndGradient(
    ...
    #     denominator += np.exp(np.dot(outsideVectors[i].T, centerWordVec))
    #
    # loss = np.exp(np.dot(outsideVectors[outsideWordIdx].T, centerWordVec))

    # As given, P(O=0, C=c) = sigmoid functoion of dot product of u_0 an v_c
    ...
    # print("gradOutsideVecs.shape", gradOutsideVecs.shape, outsideVectors.shape)
...
def negSamplingLossAndGradient(
    ...
    for i in range(K):
        w_k = indices[i+1]
        y_hat_k = sigmoid(-np.dot(outsideVectors[w_k], centerWordVec))
        loss += -np.log(y_hat_k)
        gradOutsideVecs[w_k] += np.dot(1-y_hat_k, centerWordVec)
        gradCenterVec += np.dot(1-y_hat_k, outsideVectors[w_k])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
        ...
        step_loss, grad_center_vec, grad_outside_vec = word2vecLossAndGradient(center_word_vec, outside_word_idx,
                                                                               outsideVectors, dataset)

        loss += step_loss
        gradCenterVecs[center_word_idx] += grad_center_vec
        gradOutsideVectors += grad_outside_vec
    ### END YOUR CODE
